...
class MusicAI:
    ...
    def generate_embeddings(self, track_features: List[Dict[str, Any]]) -> np.ndarray:
        """Generate embeddings for tracks using the neural encoder."""
        features = self._extract_features(track_features)
        if len(features) == 0:
            return np.array([])
