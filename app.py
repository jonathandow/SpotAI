...
class SpotAI:
    ...
    def create_playlist(self, playlist_name, num_clusters=5, num_iterations=10):
        ...
        #getting track ids for top 200 songs
        track_ids = [track['id'] for track in top_songs[:200]]


        #using spotify api to get audio features for these tracks
        try:
        ...
        #counter to keep track of how many times a song has been played
        ...
        #initializing feature columns (can change these later this is just what i found online)
        ...
        recommendations = []
        #getting the closest song to the centroid of each cluster
        ...
        #this part is just creating the playlist
        ...
        #counter for how many times a song is recommended
        rec_counter = Counter()
        #iterating through num iterations 
        for _ in range(num_iterations):
            #getting 5 random songs from recommendations
            random_tracks = random.sample(recommendations, min(5, len(recommendations)))
            try:
                #passing these 5 random songs into the spotify recommendation api to get 100 recommended songs
                recommended_tracks = self.sp.recommendations(seed_tracks=random_tracks, limit=100)
            except spotipy.exceptions.SpotifyException as e:
                return jsonify({'error': str(e)}), 500

            #checking to make sure the recommended songs are not already known
        ...
        #getting the 100 most common recommended songs and adding them to the playlist
    ...
    def create_playlist_from_playlist(self, selected_playlist_id, playlist_name, num_clusters=5, num_iterations=10):
        ...
        for item in tracks:
            if item and 'track' in item and item['track']:
                ...
                print("Invalid track item found:", item)
        ...
        if not track_ids:
            return jsonify({'error': "No tracks found in the playlist"}), 500
